...
export default function TargetWeightScreen() {
  const { gender, height, weight: currentWeight, age } = useLocalSearchParams<any>();

  const [weight, setWeight] = useState(65.0);
  const weightRef = useRef(65.0);  // always current, avoids stale closures

  const personImgSource = gender === 'female'
    ? require('@/assets/images/onboarding/woman.webp')
    : require('@/assets/images/onboarding/man.webp');

  // Reanimated SharedValue for thumb position
  // 0 = top (MAX_W), TRACK_HEIGHT = bottom (MIN_W)
  const INITIAL_POS = TRACK_HEIGHT * (1 - (65.0 - MIN_W) / (MAX_W - MIN_W));
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.titleSection}>
        <MaskedView maskElement={<Text style={styles.title}>Target Weight</Text>}>
          <LinearGradient colors={['#12879a', '#5cbf5a']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
            <Text style={[styles.title, { opacity: 0 }]}>Target Weight</Text>
          </LinearGradient>
        </MaskedView>
        <View style={styles.titleUnderline} />
        <Text style={styles.question}>What is your target weight?</Text>
        <Text style={styles.subQuestion}>Drag the slider to select your goal weight</Text>
      </View>
      ...
      <TouchableOpacity
        activeOpacity={0.85}
        onPress={() => router.push({ pathname: '/(auth)/register', params: { gender, height, currentWeight, age, targetWeight: weight } })}
      ...
      >
        ...
        >
          <Text style={styles.continueTxt}>Finish   →</Text>
        </LinearGradient>
      </TouchableOpacity>
